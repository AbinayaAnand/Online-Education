...
def checking():
    if newpass_word.get()!=conpass_word.get():
        ...
        database()
        cursor.execute("SELECT emailid FROM Candidate WHERE username=?",[user_name.get()])
        if cursor.fetchone() is not None:
            database()
            cursor.execute("UPDATE Candidate SET password=? WHERE username=?",([newpass_word.get(),user_name.get()]))
            conn.commit()
            new()
        else:
